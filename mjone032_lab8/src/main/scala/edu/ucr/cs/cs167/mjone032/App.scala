...
object App {
  ...
  def main(args : Array[String]) {
    ...
    try {
      ...
      val sentimentData: DataFrame = spark.read.format("csv").option("header", "true")
        .option("quote", "\"")
        .option("escape", "\"").load(inputfile);
...
//      sentimentData.show();
      ...
      val mypipeline = new Pipeline().setStages(Array(tokenzier, hashingTF, stringIndexer,logisticRegression))
